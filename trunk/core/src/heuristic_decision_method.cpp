...
const std::vector<Eigen::MatrixXd> &heuristic_decision_method::alternatives() const {
...
const Eigen::MatrixXd &heuristic_decision_method::criteria() const {
	return criteria_;
}
...
int heuristic_decision_method::criteria_count() const {
	return alternatives_.size();
}

int heuristic_decision_method::alternatives_count() const {
	return alternatives_.at(0).rows();
}

const Eigen::VectorXd &heuristic_decision_method::final_weights() const {
